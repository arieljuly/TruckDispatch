<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">Manage fleet trucks, driver assignments, and activity logs</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="border-b border-gray-200">
                <nav class="flex -mb-px space-x-8 px-6" aria-label="Tabs">
                    <button wire:click="setActiveTab('trucks')" 
                        class="{{ $activeTab === 'trucks' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} 
                        whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-150">
                        Trucks
                        <span class="ml-2 bg-gray-200 text-gray-700 rounded-full px-2 py-0.5 text-xs">{{ $totalTrucksCount }}</span>
                    </button>
                    <button wire:click="setActiveTab('logs')" 
                        ...
                        whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-150">
                        <svg class="inline-block -mt-1 mr-2 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                        </svg>
                        Truck Logs
                        @if($totalLogsCount > 0)
                            <span class="ml-2 bg-gray-200 text-gray-700 rounded-full px-2 py-0.5 text-xs">{{ $totalLogsCount }}</span>
                        @endif
                    </button>
                </nav>
            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <div class="flex items-end">
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" wire:model.live="showInactive" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                        <span class="ml-2 text-sm text-gray-700">Show inactive trucks</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Current Fuel</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Remaining Capacity</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse($trucks as $truck)
                                                                                                                                        <tr class="hover:bg-gray-50 transition-colors duration-150 {{ $truck->trashed() ? 'bg-gray-50 opacity-75' : '' }}">
                                                                                                                                            ...
                                                                                                                                            <td class="px-6 py-4 text-sm text-gray-900">{{ number_format($truck->compartments->sum('capacity_ltrs'), 2) }}&nbspL</td>
                                                                                                                                            <td class="px-6 py-4 text-sm text-gray-900">{{ number_format($truck->compartments->sum('loaded_ltrs'), 2) }}&nbspL</td>
                                                                                                                                            <td class="px-6 py-4 text-sm text-gray-900">{{ number_format($truck->compartments->sum('available_ltrs'), 2) }}&nbspL</td>
                                                                                                                                            ...
                                                                                                                                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                                                                                                                                <div class="flex items-center justify-end space-x-3">
                                                                                                                                                    ...
                                                                                                                                                    <!-- Assign Button -->
                                                                                                                                                    @if(!$truck->trashed() && !$truck->currentAssignment && $truck->status !== 'maintenance')
                                                                                                                                                        <button type="button" 
                                                                                                                                                    ...
                                                                                                                                                    @elseif(!$truck->trashed() && $truck->currentAssignment)
                                                                                                                                                    ...
                                                                                                                                                    <!-- Restore Button for soft-deleted trucks -->
                                                                                                                                                    @if($truck->trashed())
                                                                                                                                                        <button onclick="confirmRestore({{ $truck->id }})" 
                                                                                                                                                            class="text-green-600 hover:text-green-800 transition-colors duration-150" 
                                                                                                                                                            title="Restore Truck">
                                                                                                                                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                                                                                                    ...
                                                                                                                                                    @if(!$truck->trashed())
                                                                                                                                                        ...
                                                                                                                                                        @else
                                                                                                                                                            <button type="button" 
                                                                                                                                                                ...
                                                                                                                                                                title="Cannot delete truck with active driver assignment" 
                                                                                                                                                                disabled>
                                                                                                                                                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                                                                                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                                                                                                                                                </svg>
                        ...
                        <div class="mb-6 bg-gray-50 rounded-lg p-4">
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div>
                                    ...
                                    <select wire:model.live="logActionFilter" 
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                        <option value="">All Actions</option>
                                        <option value="created">Created</option>
                                        <option value="fuel_added">Fuel Added</option>
                                        <option value="fuel_removed">Fuel Removed</option>
                                        <option value="maintenance">Maintenance</option>
                                ...
                                <div>
                                    ...
                                    <select wire:model.live="logTruckFilter" 
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        ...
        $actionColors = [
            ...
            'fuel_added' => 'bg-green-100 text-green-800',
            'fuel_removed' => 'bg-yellow-100 text-yellow-800',
            'maintenance' => 'bg-red-100 text-red-800',
        ...
        $actionLabels = [
            ...
            'inactive' => 'Inactive',
                                                ...
                                                @endphp
                                                <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full {{ $actionColors[$log->action] ?? 'bg-gray-100 text-gray-800' }}">
                                                    {{ $actionLabels[$log->action] ?? ucfirst(str_replace('_', ' ', $log->action)) }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 text-sm text-gray-900">
                                                @if($log->liters)
                                                    {{ number_format($log->liters, 2) }} L
                                                @else
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="px-6 py-12 text-center">
                                                <div class="text-gray-500">No truck logs found.</div>
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
    ...
    <script>
        ...
        function confirmRestore(truckId) {
            Swal.fire({
                title: 'Restore Truck?',
                text: "This will restore the truck and make it available again.",
                icon: 'question',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    @this.call('restoreTruck', truckId);
                }
            });
        }
